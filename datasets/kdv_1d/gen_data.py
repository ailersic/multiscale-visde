...
def create_dataset(mu: Float[Tensor, "n_traj dim_mu"],
    ...
    x_domain = torch.linspace(0, 10, dim_x, dtype=torch.float64)
    dim_f = 1
    ...
    for i in range(n_traj):
        sol = solve_ivp(dyn, [t[i, 0], t[i, -1]], x[i, 0, :], t_eval=t[i, :], method="LSODA", rtol=1e-3, atol=1e-5)
        print(sol.message)
        x[i] = torch.tensor(sol.y).T
        global nfe
        print(f"Trajectory {i}: cumulative NFE = {nfe}, running avg NFE per traj = {nfe / (i+1)}")
...
def main():
    ...
    T = 1.0
    mu = torch.cat([torch.full([n_traj, 1], 0.022)], dim=1)
    ...
    train_mu, train_t, train_x, train_f = mu[:n_traj_train], t[:n_traj_train], x[:n_traj_train].unsqueeze(2), f[:n_traj_train]
    val_mu, val_t, val_x, val_f = mu[n_traj_train:n_traj_train + n_traj_val], t[n_traj_train:n_traj_train + n_traj_val], x[n_traj_train:n_traj_train + n_traj_val].unsqueeze(2), f[n_traj_train:n_traj_train + n_traj_val]
    test_mu, test_t, test_x, test_f = mu[n_traj_train + n_traj_val:], t[n_traj_train + n_traj_val:], x[n_traj_train + n_traj_val:].unsqueeze(2), f[n_traj_train + n_traj_val:]
    ...
    with open(os.path.join(CURR_DIR, "nfe.txt"), "w") as f:
        f.write(f"Avg NFE per traj = {nfe / n_traj}")
