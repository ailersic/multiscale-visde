...
def main(dim_z_macro: int = 20, dim_z_micro: int = 5, n_sigma: int = 3, max_epochs: int = 500, lr: float = 1e-3, lr_sched_freq: int = 1000):
    ...
    for i_traj in range(n_traj):
        ...
        for j, j_t in enumerate(tsamples):
            ...
            ax.plot(np.linspace(0, 1, dim_x), x_true[j_t], color='black', linewidth=4)
            ax.plot(np.linspace(0, 1, dim_x), x_mean, color='red', linestyle='--', linewidth=4)
            ax.fill_between(np.linspace(0, 1, dim_x), x_mean - x_std, x_mean + x_std, alpha=0.3, color='red')
