    ...
    def __init__(self, config, dim_z_macro, dim_z_micro, shared_kernel, shared_bias, n_sigma):
        ...
        if self.dim_z_micro > 0:
            self.micro_net = nn.Sequential(nn.Unflatten(1, self.shape_x),
                                           ...
                                           nn.LeakyReLU(),
                                           nn.Flatten(),
                                           nn.Linear(64*2*2, self.dim_z_micro)
                                           )
...
class DecodeMeanNet(nn.Module):
    def __init__(self, config, dim_z_macro, dim_z_micro, shared_kernel, shared_bias, n_sigma, _macro_var, _micro_var):
        super(DecodeMeanNet, self).__init__()

        self.dim_x = config.dim_x
        self.shape_x = config.shape_x
        n_chan = self.shape_x[0]
        grid_x = self.shape_x[1]

        self.dim_z_macro = dim_z_macro
        self.dim_z_micro = dim_z_micro
        self.sigma = int(np.sqrt(self.dim_x//self.dim_z_macro))
        grid_z = grid_x // self.sigma

        self.macro_net = nn.Sequential(nn.Unflatten(1, (n_chan, grid_z, grid_z)),
        ...
        if self.dim_z_micro > 0:
            self.micro_net = nn.Sequential(nn.Linear(self.dim_z_micro, 64*2*2),
                                           nn.Unflatten(1, (64, 2, 2)),
                                           nn.LeakyReLU(),
                                           nn.ConvTranspose2d(64, 32, kernel_size=micro_kernel, stride=4, padding=micro_padding, output_padding=3),
                                           nn.LeakyReLU(),
                                           nn.ConvTranspose2d(32, 16, kernel_size=micro_kernel, stride=4, padding=micro_padding, output_padding=3),
                                           nn.LeakyReLU(),
                                           nn.ConvTranspose2d(16, n_chan, kernel_size=micro_kernel, stride=4, padding=micro_padding, output_padding=3),
                                           nn.Flatten()
                                           )
...
class DecodeVarNet(nn.Module):
    def __init__(self, config, dim_z_macro, dim_z_micro, _macro_var, _micro_var):
            ...
            for j in range(grid_z):
                for ir in range(i - self.r, i + self.r + 1):
                    for jr in range(j - self.r, j + self.r + 1):
                        _adj[i, j, ir % grid_z, jr % grid_z] = True
        _adj = _adj.reshape(grid_z**2, grid_z**2)

        self.adj = _adj.nonzero(as_tuple=True)[1].reshape(grid_z**2, (2*self.r + 1)**2)

        self.macro_net = nn.Sequential(nn.Linear(self.n_chan*(2*self.r + 1)**2 + self.dim_z_micro, 128),
                                        nn.ReLU(),
                                        nn.Linear(128, 512),
                                        nn.ReLU(),
                                        nn.Linear(512, 128),
...
def create_latent_sde(dim_z_macro: int,
    ...
    shared_kernel = nn.Parameter((2*torch.rand(n_chan, n_chan, 2*n_sigma*sigma + 1, 2*n_sigma*sigma + 1) - 1)/
                                    np.sqrt(n_chan*n_chan*(2*n_sigma*sigma + 1)*(2*n_sigma*sigma + 1)))
    #kernel_range_x = torch.arange(-n_sigma*sigma, n_sigma*sigma + 1).view(1, 1, -1, 1).tile((n_chan, n_chan, 1, 1))
    #kernel_range_y = torch.arange(-n_sigma*sigma, n_sigma*sigma + 1).view(1, 1, 1, -1).tile((n_chan, n_chan, 1, 1))
    ...
    # decoder        
    decode_mean_net = DecodeMeanNet(vaeconfig, dim_z_macro, dim_z_micro, shared_kernel, shared_bias, n_sigma, _macro_var, _micro_var)
    decode_var_net = DecodeVarNet(vaeconfig, dim_z_macro, dim_z_micro, _macro_var, _micro_var)
