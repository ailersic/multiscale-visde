...
def main(dim_z_macro: int = 64, dim_z_micro: int = 2, max_epochs: int = 100, lr: float = 1e-3, lr_sched_freq: int = 1000):
    ...
    fig = plt.figure(figsize=(12*n_traj, 3*len(tsamples)))
    ...
    for i_traj in range(n_traj):
        ...
        for j, j_t in enumerate(tsamples):
            ...
            axgrid[id1].imshow(x_true[j_t, 0], cmap=cmap, vmin=state_min, vmax=state_max)
            ...
            axgrid[id2].imshow(x_mean[0], cmap=cmap, vmin=state_min, vmax=state_max)
            ...
            axgrid[id3].imshow(x_macro[0], cmap=cmap, vmin=state_min, vmax=state_max)
            ...
            if dim_z_micro > 0:
                axgrid[id4].imshow(x_micro[0], cmap=cmap, vmin=state_min, vmax=state_max)
            ...
            axgrid[id5].imshow(np.abs(x_true[j_t, 0] - x_mean[0]), cmap='coolwarm', vmin=error_min, vmax=error_max)
            ...
            axgrid[id6].imshow(x_std[0], cmap='coolwarm', vmin=error_min, vmax=error_max)
        ...
        t_macroplot = n_tsteps//2
        full_img = x[i_traj,t_macroplot,0].cpu().detach().numpy()
        smooth_img = model.encoder.encode_mean.smooth_net(x[i_traj,t_macroplot].unsqueeze(0).flatten(1)).reshape(128, 128).cpu().detach().numpy()
        macro_img = model.encoder.encode_mean.macro_net(x[i_traj,t_macroplot].unsqueeze(0).flatten(1)).reshape(8, 8).cpu().detach().numpy()
        ...
        axmacro.axis('off')
        figmacro.savefig(os.path.join(out_dir, f"{TRAIN_VAL_TEST}_full_traj_{i_traj}.pdf"), bbox_inches='tight')
        plt.close()

        figmacro, axmacro = plt.subplots(figsize=(6, 6))
        axmacro.imshow(smooth_img*(full_max - full_min)/(smooth_max - smooth_min), cmap=cmap, vmin=all_min, vmax=all_max)
        axmacro.axis('off')
        figmacro.savefig(os.path.join(out_dir, f"{TRAIN_VAL_TEST}_smooth_traj_{i_traj}.pdf"), bbox_inches='tight')
        plt.close()

        figmacro, axmacro = plt.subplots(figsize=(6, 6))
        axmacro.imshow(resid_img, cmap=cmap, vmin=all_min, vmax=all_max)
        axmacro.axis('off')
        figmacro.savefig(os.path.join(out_dir, f"{TRAIN_VAL_TEST}_resid_traj_{i_traj}.pdf"), bbox_inches='tight')
        plt.close()

        figmacro, axmacro = plt.subplots(figsize=(6, 6))
        axmacro.imshow(macro_img*(full_max - full_min)/(smooth_max - smooth_min), cmap=cmap, vmin=all_min, vmax=all_max)
        axmacro.axis('off')
        figmacro.savefig(os.path.join(out_dir, f"{TRAIN_VAL_TEST}_macro_traj_{i_traj}.pdf"), bbox_inches='tight')
        plt.close()
