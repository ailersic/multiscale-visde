...
def main(dim_z_macro: int = 512,
    ...
    for j, j_t in enumerate(tsamples):
        ...
        for chan in range(2):
            ...
            fig, ax = plt.subplots(figsize=(5, 2))
            ax.imshow(np.abs(x_mean[chan] - x_true[j_t, chan]), cmap="afmhot", vmin=0.0, vmax=1.0)
            plt.axis("off")
            fig.tight_layout()
            ...
            fig, ax = plt.subplots(figsize=(5, 2))
            ax.imshow(x_std[chan], cmap="afmhot", vmin=0.0, vmax=1.0)
            plt.axis("off")
            fig.tight_layout()
