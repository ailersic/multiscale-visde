...
def main(config: CaseConfig = DEFAULT_CONFIG) -> None:
    ...
    for j, j_t in enumerate(tsamples):
        ...
        ax.imshow(x_true[j_t, 0], cmap=cmap, vmin=-0.22, vmax=1.06)
        plt.axis("off")
        fig.tight_layout()
        fig.savefig(os.path.join(out_dir, f"{TRAIN_VAL_TEST}_{i_traj}_{j}_true.pdf"), format='pdf')
        fig.show()

        fig, ax = plt.subplots(figsize=(3, 3))
        ax.imshow(x_mean[0], cmap=cmap, vmin=-0.22, vmax=1.06)
        plt.axis("off")
        fig.tight_layout()
        fig.savefig(os.path.join(out_dir, f"{TRAIN_VAL_TEST}_{i_traj}_{j}_pred.pdf"), format='pdf')
        fig.show()

        fig, ax = plt.subplots(figsize=(3, 3))
        ax.imshow(np.abs(x_true[j_t, 0] - x_mean[0]), cmap="afmhot", vmin=0, vmax=0.18)
        plt.axis("off")
        fig.tight_layout()
        fig.savefig(os.path.join(out_dir, f"{TRAIN_VAL_TEST}_{i_traj}_{j}_err.pdf"), format='pdf')
        fig.show()

        fig, ax = plt.subplots(figsize=(3, 3))
        ax.imshow(x_std[0], cmap="afmhot", vmin=0, vmax=0.18)
        plt.axis("off")
        fig.tight_layout()
        fig.savefig(os.path.join(out_dir, f"{TRAIN_VAL_TEST}_{i_traj}_{j}_std.pdf"), format='pdf')
        fig.show()
