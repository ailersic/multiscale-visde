...
from experiments_refac.cylinder_2d.utils import CaseConfig, get_version_str
...
class DriftMacroNetGraph(nn.Module):
    def __init__(self, config, shape_z, dim_z_macro, dim_z_micro_activ):
        ...
        self.macro_net = nn.Sequential(nn.Linear(self.shape_z[0]*(2*self.r + 1)**2 + 4 + self.dim_z_micro_activ, 128),
                                       nn.LeakyReLU(),
                                       nn.Linear(128, 256),
                                       nn.LeakyReLU(),
                                       nn.Linear(256, 128),
...
class DriftMicroNet(nn.Module):
    def __init__(self, config, shape_z, dim_z_micro, dim_z_macro_activ):
        ...
        self.micro_net = nn.Sequential(nn.Linear(dim_z_micro + dim_z_macro_activ, 128),
                                       nn.LeakyReLU(),
                                       nn.Linear(128, 256),
                                       nn.LeakyReLU(),
                                       nn.Linear(256, 128),
    ...
    def forward(self,
        ...
        dzdt_micro = self.micro_net(torch.cat([z_micro, z_macro_activ], dim=1))
...
class DriftMicroActivNet(nn.Module):
    def __init__(self, config, dim_z_micro):
        ...
        self.micro_activ_net = nn.Sequential(nn.Linear(dim_z_micro, 128),
                                             nn.LeakyReLU(),
                                             nn.Linear(128, 128),
                                             nn.LeakyReLU(),
                                             nn.Linear(128, dim_z_micro)
    ...
    def forward(self,
        ...
        return z_micro + self.micro_activ_net(z_micro)
...
def augment_latent_sde(old_config: CaseConfig, config: CaseConfig, device: torch.device = torch.device("cuda:0")) -> visde.LatentSDE:
    ...
    if old_config.dim_z_micro >= 0:
        ...
        with torch.no_grad():
            ...
            # drift
            new_weight = torch.zeros((128, 54 + config.dim_z_micro)) # 29 if ij included
            new_weight[:, :54 + config.dim_z_micro - 1] = state_dict["drift.macro_drift_net.macro_net.0.weight"]
            torch.nn.init.xavier_normal_(new_weight[:, 54 + config.dim_z_micro - 1:])
            state_dict["drift.macro_drift_net.macro_net.0.weight"] = new_weight

            new_weight = torch.zeros((128, 2*config.dim_z_micro))
            new_weight[:, :2*(config.dim_z_micro-1)] = state_dict["drift.micro_drift_net.micro_net.0.weight"]
            torch.nn.init.xavier_normal_(new_weight[:, 2*(config.dim_z_micro-1):])
            ...
            new_weight = torch.zeros((128, config.dim_z_micro))
            new_weight[:, :config.dim_z_micro-1] = state_dict["drift.micro_activ_net.micro_activ_net.0.weight"]
            torch.nn.init.xavier_normal_(new_weight[:, config.dim_z_micro-1:])
            state_dict["drift.micro_activ_net.micro_activ_net.0.weight"] = new_weight

            new_weight = torch.zeros((config.dim_z_micro, 128))
            new_weight[:config.dim_z_micro-1] = state_dict["drift.micro_activ_net.micro_activ_net.4.weight"]
            torch.nn.init.xavier_normal_(new_weight[config.dim_z_micro-1:])
            state_dict["drift.micro_activ_net.micro_activ_net.4.weight"] = new_weight

            new_bias = torch.zeros((config.dim_z_micro,))
            new_bias[:config.dim_z_micro-1] = state_dict["drift.micro_activ_net.micro_activ_net.4.bias"]
            torch.nn.init.zeros_(new_bias[config.dim_z_micro-1:])
            state_dict["drift.micro_activ_net.micro_activ_net.4.bias"] = new_bias
            ...
            if old_config.dim_z_micro >= 1:
                new_weight = torch.zeros((config.dim_z_micro, 6400))
                ...
                new_weight = torch.zeros((6400, config.dim_z_micro))
