...
dmd = DMD(svd_rank=517, tikhonov_regularization=0.01)
dmd.fit(snapshots)
...
tsamples = [0, x_test.shape[1]//4, x_test.shape[1]//2, 3*x_test.shape[1]//4, x_test.shape[1]-1]
...
for i in range(x_test.shape[0]):
    ...
    if i == 0:
        for j_plot, j_t in enumerate(tsamples):
            ...
            ax.imshow(x_i_dmd[j_t].reshape(x.shape[2:])[0], cmap=cmap, vmin=-1, vmax=1.5)
            plt.axis("off")
            fig.tight_layout()
            fig.savefig(os.path.join(out_dir, f"test_{i}_{j_plot}_pred_0.pdf"), format='pdf')
            fig.show()

            fig, ax = plt.subplots(figsize=(5, 2))
            ax.imshow(x_i_dmd[j_t].reshape(x.shape[2:])[1], cmap=cmap, vmin=-1, vmax=1.5)
            plt.axis("off")
            fig.tight_layout()
            fig.savefig(os.path.join(out_dir, f"test_{i}_{j_plot}_pred_1.pdf"), format='pdf')
            fig.show()
