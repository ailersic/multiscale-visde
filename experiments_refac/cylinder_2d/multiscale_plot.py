...
def main(config: CaseConfig = DEFAULT_CONFIG) -> None:
    ...
    for j, j_t in enumerate(tsamples):
        for chan in range(2):
            x_min[chan] = -1.0#min(x_min[chan], x[i_traj, j_t, chan].min())
            x_max[chan] = 1.5#max(x_max[chan], x[i_traj, j_t, chan].max())
    ...
    for tsample in tsamples:
        ...
        for chan in range(2):
            ...
            ax.imshow(x[i_traj, tsample, chan].cpu().detach().numpy(), cmap=cmap, vmin=x_min[chan], vmax=x_max[chan])
            plt.axis('off')
            plt.tight_layout()
            plt.savefig(os.path.join(out_dir, f"trajectory_{i_traj}_tsample_{tsample}_true_{chan}.pdf"))
            plt.close()

            fig, ax = plt.subplots(figsize=(6, 2))
            ax.imshow(xr_i[0, chan].cpu().detach().numpy(), cmap=cmap, vmin=x_min[chan], vmax=x_max[chan])
            plt.axis('off')
            plt.tight_layout()
            plt.savefig(os.path.join(out_dir, f"trajectory_{i_traj}_tsample_{tsample}_recon_{chan}.pdf"))
            plt.close()

            fig, ax = plt.subplots(figsize=(6, 2))
            ax.imshow(x_smooth[0, chan].cpu().detach().numpy(), cmap=cmap, vmin=x_min[chan], vmax=x_max[chan])
            plt.axis('off')
            plt.tight_layout()
            plt.savefig(os.path.join(out_dir, f"trajectory_{i_traj}_tsample_{tsample}_smooth_{chan}.pdf"))
            plt.close()

            fig, ax = plt.subplots(figsize=(6, 2))
            ax.imshow(x_resid[0, chan].cpu().detach().numpy(), cmap=cmap, vmin=x_min[chan], vmax=x_max[chan])
            plt.axis('off')
            plt.tight_layout()
            plt.savefig(os.path.join(out_dir, f"trajectory_{i_traj}_tsample_{tsample}_resid_{chan}.pdf"))
            plt.close()
