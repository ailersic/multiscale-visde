...
from experiments_refac.cylinder_2d.sindy_2step.def_model import create_sindy_model
    ...
    for i in range(n_traj):
        ...
        if i == 0:
            for j_plot, j_t in enumerate(tsamples):
                ...
                ax.imshow(x_sim_i[j_t, 0], cmap=cmap, vmin=-1, vmax=1.5)
                plt.axis("off")
                fig.tight_layout()
                ...
                fig, ax = plt.subplots(figsize=(5, 2))
                ax.imshow(x_sim_i[j_t, 1], cmap=cmap, vmin=-1, vmax=1.5)
                plt.axis("off")
                fig.tight_layout()
