...
def plot_pred_vs_true(traj_dir: str,
    ...
    for i_chan in range(x_true.shape[1]):
        fig, axgrid = plt.subplots(nrows=len(tsamples), ncols=6, figsize=(18, 3*len(tsamples)))
        ...
        error_min, error_max = state_min, state_max

        state_diff = state_max - state_min
        error_diff = error_max - error_min
        state_min -= 0.1 * state_diff
        state_max += 0.1 * state_diff
        error_min -= 0.1 * error_diff
        error_max += 0.1 * error_diff

        for j, j_t in enumerate(tsamples):
            axgrid[j, 0].plot(x_true[j_t, i_chan])
            axgrid[j, 0].set_ylim(state_min, state_max)
            ...
            axgrid[j, 1].plot(x_mean[j_t, i_chan])
            axgrid[j, 1].set_ylim(state_min, state_max)
            ...
            axgrid[j, 2].plot(x_macro[j_t, i_chan])
            axgrid[j, 2].set_ylim(state_min, state_max)
            ...
            if x_micro is not None:
                axgrid[j, 3].plot(x_micro[j_t, i_chan])
                axgrid[j, 3].set_ylim(state_min, state_max)
            ...
            axgrid[j, 4].plot(np.abs(x_true[j_t, i_chan] - x_mean[j_t, i_chan]))
            axgrid[j, 4].set_ylim(error_min, error_max)
            ...
            axgrid[j, 5].plot(x_std[j_t, i_chan])
            axgrid[j, 5].set_ylim(error_min, error_max)
...
def main(config: CaseConfig = DEFAULT_CONFIG) -> None:
    ...
    with open(os.path.join(out_dir, "nfe.txt"), "w") as file:
