...
from experiments_refac.kdv_1d.sindy_2step.def_model import create_sindy_model
...
def main(data_file: str = "data_10_5_5",
        dim_z: int = 25,
    ...
    version = "_".join([data_file.split(".")[0], str(dim_z), str(threshold), str(degree)])
    ...
    for i in range(n_traj):
        ...
        nfe[i] = counter['count']
        #z_sim = model.simulate(z_i[0], t=t_i, integrator_kws={'atol': 1e-8, 'method': 'RK45', 'rtol': 1e-6})
        ...
        if i == 0:
            for j_plot, j_t in enumerate(tsamples):
                ...
                ax.plot(np.linspace(0, 1, x_i[j_t, 0].shape[0]), x_i[j_t, 0], color='black', linewidth=4)
                ax.plot(np.linspace(0, 1, x_sim_i[j_t, 0].shape[0]), x_sim_i[j_t, 0], color='red', linestyle='--', linewidth=4)
    ...
    with open(os.path.join(out_dir, f"{TRAIN_VAL_TEST}_error.txt"), "w") as file:
        file.write(np.array2string(rel_err, precision=5))
        file.write("\n")
        file.write(f"Mean: {np.mean(rel_err):.5f}, Std Dev: {np.std(rel_err):.5f}\n")
