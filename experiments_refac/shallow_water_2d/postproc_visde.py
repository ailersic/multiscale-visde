...
def plot_pred_vs_true(traj_dir: str,
    ...
    for i_chan in range(x_true.shape[1]):
        fig, axgrid = plt.subplots(nrows=len(tsamples), ncols=6, figsize=(18, 3*len(tsamples)), sharex=True, sharey=True)
...
def main(config: CaseConfig = DEFAULT_CONFIG) -> None:
    #full_path = full_data_path(config.data_file[:-6]) # evaluate on clean dataset
    full_path = full_data_path(config.data_file) # evaluate on noisy dataset
    dataset = visde.MultiTrajHDF5(full_path, TRAIN_VAL_TEST, config.n_win)
    sampler = visde.MultiTemporalSampler(dataset, dataset.n_tsteps, random=False)
    ...
    n_tsteps = dataset.n_tsteps
    n_batch_encoder = 16
    n_batch_decoder = 64

    every_n_traj = 1
    rel_err = np.zeros((n_traj//every_n_traj, n_tsteps))
    sq_err = np.zeros((n_traj//every_n_traj, n_tsteps, dim_x))
    aenc_rel_err = np.zeros((n_traj//every_n_traj, n_tsteps))
    ...
    with torch.no_grad():
        for i_traj, batch_traj in enumerate(loader):
            if i_traj % every_n_traj != 0:
                continue
            ...
            mu, t, x_win, x, f = batch_traj

            mu_i = mu[0:1].to(device)
            mu_i_batch = mu_i.repeat((n_batch_encoder, 1))
            x0_i = x_win[0:1].to(device)
            t_i = t.to(device)
            f_i = f.to(device)
            ...
            x_true = torch.Tensor(x).to(device)
            ...
            for j_t in range(n_tsteps):
                ...
                rel_err[i_traj//every_n_traj, j_t] = ((x_mean[j_t] - x_true[j_t]).pow(2).sum() / x_true[j_t].pow(2).sum()).sqrt().item()
                sq_err[i_traj//every_n_traj, j_t] = ((x_mean[j_t] - x_true[j_t]).pow(2)).flatten().cpu().detach().numpy()

                z_enc[j_t], _ = model.encoder(mu_i, x_true[j_t:(j_t+config.n_win)].unsqueeze(0))
                x_rec_ij, _ = model.decoder(mu_i, z_enc[j_t])

                aenc_rel_err[i_traj//every_n_traj, j_t] = ((x_rec_ij - x_true[j_t]).pow(2).sum() / x_true[j_t].pow(2).sum()).sqrt().item()
            print("done", flush=True)
            ...
            plot_error(traj_dir,
                    t_i.cpu().detach().numpy(),
                    aenc_rel_err[i_traj//every_n_traj],
                    rel_err[i_traj//every_n_traj]
                    )
            ...
            with open(os.path.join(traj_dir, "error.txt"), "w") as file:
                ...
                file.write(np.array2string(rel_err[i_traj//every_n_traj], precision=5))
                file.write("\n")
                file.write(f"Mean: {np.mean(rel_err[i_traj//every_n_traj]):.5f}, Std Dev: {np.std(rel_err[i_traj//every_n_traj]):.5f}\n")

            print(f"Error Mean: {np.mean(rel_err[i_traj//every_n_traj])}, Std Dev: {np.std(rel_err[i_traj//every_n_traj])}", flush=True)
            print("-"*80, flush=True)
        ...
        with open(os.path.join(out_dir, f"{TRAIN_VAL_TEST}_nfe.txt"), "w") as file:
            file.write(f"Total NFE: {model.drift.macro_drift_net.nfe}")
            file.write("\n")
            file.write(f"Avg NFE per traj: {model.drift.macro_drift_net.nfe / n_traj}")
        ...
        with open(os.path.join(out_dir, f"{TRAIN_VAL_TEST}_rmse.txt"), "w") as file:
            file.write(f"RMSE: {np.mean(np.sqrt(np.mean(sq_err, axis=-1))):.5f}\n")
            # RMS for a state at a given point in time, avg over time and trajectories
